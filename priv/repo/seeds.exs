...
get_or_update_user = fn email, display_name, location, lat, lng ->
  case Accounts.get_user_by_email(email) do
    ...
    existing_user ->
      {:ok, updated_user} =
        Accounts.update_user_location(existing_user, %{
          "location" => location,
          "latitude" => lat,
          "longitude" => lng
        })
...
case Repo.get_by(Message, id: 2) do
  nil ->
    Repo.insert!(%Message{
      id: 2,
      content:
        "That's great Maria! We're meeting this Saturday at 10am. Would you like to join us?",
      sender_id: jonas.id,
      recipient_id: maria.id,
      inserted_at:
        DateTime.utc_now()
        |> DateTime.add(-3 * 24 * 60 * 60 + 30 * 60, :second)
        |> DateTime.truncate(:second),
      updated_at:
        DateTime.utc_now()
        |> DateTime.add(-3 * 24 * 60 * 60 + 30 * 60, :second)
      ...
      read_at:
        DateTime.utc_now()
        |> DateTime.add(-3 * 24 * 60 * 60 + 2 * 60 * 60, :second)
        |> DateTime.truncate(:second)
    })
...
case Repo.get_by(Message, id: 3) do
  nil ->
    Repo.insert!(%Message{
      ...
      read_at:
        DateTime.utc_now()
        |> DateTime.add(-2 * 24 * 60 * 60 + 1 * 60 * 60, :second)
        |> DateTime.truncate(:second)
    })
...
case Repo.get_by(Message, id: 4) do
  nil ->
    Repo.insert!(%Message{
      id: 4,
      content:
        "Just bring gloves if you have them. We have all the tools needed. Looking forward to seeing you there!",
      sender_id: jonas.id,
      recipient_id: maria.id,
      inserted_at:
        DateTime.utc_now()
        |> DateTime.add(-2 * 24 * 60 * 60 + 2 * 60 * 60, :second)
        |> DateTime.truncate(:second),
      updated_at:
        DateTime.utc_now()
        |> DateTime.add(-2 * 24 * 60 * 60 + 2 * 60 * 60, :second)
      ...
      read_at:
        DateTime.utc_now()
        |> DateTime.add(-1 * 24 * 60 * 60, :second)
        |> DateTime.truncate(:second)
    })
...
case Repo.get_by(Message, id: 5) do
  nil ->
    Repo.insert!(%Message{
      id: 5,
      content:
        "Hello Jonas, I'm organizing a workshop on sustainable living next month. Would you be interested in speaking about urban gardening?",
      sender_id: thomas.id,
      recipient_id: jonas.id,
      inserted_at:
        DateTime.utc_now()
...
case Repo.get_by(Message, id: 6) do
  nil ->
    Repo.insert!(%Message{
      id: 6,
      content:
        "Hi Jonas, I wanted to ask if you'll be attending the book club meeting next week?",
      sender_id: lisa.id,
      recipient_id: jonas.id,
      inserted_at:
        DateTime.utc_now() |> DateTime.add(-5 * 60 * 60, :second) |> DateTime.truncate(:second),
      updated_at:
        DateTime.utc_now() |> DateTime.add(-5 * 60 * 60, :second) |> DateTime.truncate(:second)
    })
